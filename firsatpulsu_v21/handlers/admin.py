...
_YARDIM = (
    ...
    "/teshis — Canlı sistem teşhisi (her modül çalışıyor mu?)\n"
    ...
    "/temizle_zehir — Model zehirlenmesini temizle (kirli veri)\n"
...
async def _teshis_panosu(event) -> None:
    """v23.5 — Canlı teşhis: her kritik modülü test eder, çalışıyor mu gösterir.
    Sağlamlık + görünürlük: bozuk/ölü özellikleri yakalar."""
    try:
        await event.reply("🔬 Teşhis çalışıyor, modüller test ediliyor…", parse_mode="html")
        from utils import teshis
        ozet = teshis.ozet()
        satirlar = ["🔬 <b>SİSTEM TEŞHİSİ</b>", ""]
        for s in ozet["detay"]:
            ikon = {"ok": "✅", "uyari": "⚠️", "hata": "❌"}[s["durum"]]
            ad = s["ad"]
            if s["durum"] == "hata":
                satirlar.append(f"{ikon} <b>{ad}</b>\n   ↳ {s['detay']}")
            else:
                satirlar.append(f"{ikon} {ad}")
        satirlar.append("")
        if ozet["hata"] == 0:
            satirlar.append(f"✅ <b>Tüm sistemler çalışıyor</b> ({ozet['ok']}/{ozet['toplam']})")
        else:
            satirlar.append(
                f"⚠️ <b>{ozet['hata']} sistemde HATA var!</b> "
                f"({ozet['ok']} çalışıyor, {ozet['uyari']} uyarı)")
        await event.reply("\n".join(satirlar), parse_mode="html")
    except Exception as e:
        await event.reply(f"⚠️ Teşhis hatası: {e}", parse_mode="html")
