...
async def telegram_yukle(tg_client) -> None:
    ...
    if toplam > 0:
        ...
        # Mevcut Telegram backup mesaj ID'sini bul. v23.4: birden fazla eski
        # yedek mesajı varsa en yenisini tut, eskileri SİL (Kaydedilenler
        # temiz kalsın — kullanıcıyı rahatsız eden çoğalmış JSON'lar gitsin).
        if config.ADMIN_ID:
            try:
                ...
                async for msg in tg_client.iter_messages(admin_id, limit=300):
                    if msg.text and (msg.text.startswith(_VERI_BASLIK)
                                     or msg.text.startswith("##FIRSATPULSU_IST_V1##")):
                        yedek_idler.append(msg.id)
                if yedek_idler:
                    # En yüksek ID = en yeni → onu tut
                    _ist_msg_id = max(yedek_idler)
                    eskiler = [i for i in yedek_idler if i != _ist_msg_id]
                    if eskiler:
                        try:
                            await tg_client.delete_messages(admin_id, eskiler)
                            log("OK", f"İstatistik: {len(eskiler)} eski yedek temizlendi")
                        except Exception:
                            pass
            except Exception:
                pass
        ...
        try:
            ...
            if _ist_msg_id:
                try:
                    ...
                    # v23.4: edit başarısız (mesaj silinmiş olabilir). Yeni mesaj
                    # atmadan ÖNCE mevcut bir yedek var mı diye bak — çoğalmasın.
                    bulundu = False
                    try:
                        async for msg in _tg_client.iter_messages(admin_id, limit=100):
                            if msg.text and msg.text.startswith(_VERI_BASLIK):
                                _ist_msg_id = msg.id
                                await _tg_client.edit_message(admin_id, msg.id, metin)
                                bulundu = True
                                break
                    except Exception:
                        pass
